...
class MasterBoardTask:
    def __init__(self, trajectory_queue, message_queue, shared_position, masterboard_started_event):
        ...
        if self.debug_plot:
            self.plotter = pltr.Plotter(f"Via points test | kp = {config.kp}, kd = {config.kd}, current saturation = {config.current_saturation}")

            for i in range(config.N_SLAVES_CONTROLLED * 2):
                self.plotter.create_plot(f"Joint {i}, Position Reference", "Position [rad]")
                self.plotter.create_plot(f"Joint {i}, Position Reading", "Position [rad]")
                self.plotter.create_plot(f"Joint {i}, Velocity Reference", "Velocity [rad/s]")
    ...
    def run(self):
        ...
        if self.debug_plot:
            ...
            self.plotter.create_axis([f"Joint {0}, Position Reference", f"Joint {0}, Position Reading"])
            self.plotter.create_axis([f"Joint {2}, Position Reference", f"Joint {2}, Position Reading"])
            self.plotter.create_axis([f"Joint {4}, Position Reference", f"Joint {4}, Position Reading"])
            self.plotter.create_axis([f"Joint {6}, Position Reference", f"Joint {6}, Position Reading"])
            self.plotter.create_axis([f"Joint {1}, Position Reference", f"Joint {1}, Position Reading"])
            self.plotter.create_axis([f"Joint {3}, Position Reference", f"Joint {3}, Position Reading"])
            self.plotter.create_axis([f"Joint {5}, Position Reference", f"Joint {5}, Position Reading"])
            self.plotter.create_axis([f"Joint {7}, Position Reference", f"Joint {7}, Position Reading"])
