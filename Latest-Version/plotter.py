...
class Plotter:
    ...
    def plot(self, specified_entries=[], grid=False):
        ...
        for keys in entries_to_plot: #Keys is now a list of entries, corresponing to an axis
            ...
            for key in keys:
                ...
                #major_ticks = np.arange(0, x_data, 20)
                #minor_ticks = np.arange(0, x_data, 5)

                #axes.set_xticks((0, 100))
                #axes.set_xticks(minor_ticks, minor=True)
                #axes.set_yticks((0, x_data))
                #axes.set_yticks(minor_ticks, minor=True)
                

                axes.plot(x_data, y_data, label=key)
                if grid:
                    axes.grid(b=True, which='both', color='dimgrey', linestyle='dashed', linewidth=0.5)
