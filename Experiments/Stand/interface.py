...
class MasterBoardTask:
    def __init__(self, trajectory_queue):
        ...
        self.plotter = Plotter(f"Stand config test | kp = {Config.kp}, kd = {Config.kd}, current saturation = {Config.current_saturation} A")
        ...
        for i in range(Config.num_joints):
            ...
            self.plotter.create_plot(f"Joint {i}, Current Reference", "Current [Ampere]")
    ...
    def run(self):
        ...
        for i in range(Config.num_joints // 2):
            ...
            self.plotter.create_axis([f"Joint {2*i + 1}, Position Reading", f"Joint {2*i + 1}, Position Reference"])
            self.plotter.create_axis([f"Joint {2*i + 1}, Current Reference"])

            self.plotter.create_axis([f"Joint {2 * i}, Current Reference"])
            self.plotter.create_axis([f"Joint {2 * i}, Position Reading", f"Joint {2 * i}, Position Reference"])
