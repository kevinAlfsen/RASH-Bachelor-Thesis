...
class MasterBoard:
    def __init__(self, plot):
        ...
        if ready_wait_time < 1:
            ...
            print(f"Zero Calibration set to : {self.zero_calibration}")
            print(f"Floor Calibration set to : {self.floor_calibration}")
        else:
    ...
    def track_reference(self, limp=False):
        ...
        for i in self.motors_spi_connected_indexes:
            ...
            if self.robot_if.GetMotor(i).IsEnabled():
                ...
                #plot.add_data(f"Joint {i}, Current Reference", current_ref)
                self.plot.add_data(f"Joint {i}, Position Reference", pos_ref[i])
                self.plot.add_data(f"Joint {i}, Position Reading", current_position)
                #plot.add_data(f"Joint {i}, Position Error", current_position - pos_ref[i])
